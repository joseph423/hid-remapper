...
### Production serial output

The main `remapper.uf2` builds with `TPS43_DIAGNOSTICS=OFF` by default. It emits no periodic TPS43 status, startup capture prompts, touch/sample traces, USB callback logs, or expression debug prints, and it does not poll serial commands or update diagnostic captures in its main loop. CDC support remains available for one-time startup, configuration, and validation errors. Build a separate diagnostic UF2 with `-DTPS43_DIAGNOSTICS=ON` to restore the serial commands, captures, and status output; `TPS43_PRINT_STATS` and `TPS43_ENABLE_SERIAL_RATE_OVERRIDE` require that option. Output queue overflow is silent in the quiet profile but retains its existing discard behavior. The quiet profile does not change HID behavior or the saved configuration format.
