...
static_assert(sizeof(persist_config_v22_t) == sizeof(persist_config_v18_t) + kTps43TuningBlockV3Size);
static_assert(sizeof(persist_config_v23_t) == sizeof(persist_config_v18_t) + kTps43TuningBlockV4Size);
...
void load_config_v23(const uint8_t* persisted_config) {
    const persist_config_v23_t* config = (const persist_config_v23_t*) persisted_config;
    DualTps43Tuning tuning;
    if (!decode_tps43_tuning(config->tps43_tuning, sizeof(config->tps43_tuning), &tuning)) {
        printf("invalid TPS43 tuning block; using production defaults\n");
        tuning = production_tuning();
    ...
    set_configured_tps43_tuning(tuning);
    load_config_v18_or_v19(persisted_config, &config->base, sizeof(persist_config_v23_t));
}

void load_config_v24(const uint8_t* persisted_config) {
    const persist_config_v24_t* config = (const persist_config_v24_t*) persisted_config;
    DualTps43Tuning tuning;
    if (!decode_tps43_tuning(config->tps43_tuning, sizeof(config->tps43_tuning), &tuning)) {
        printf("invalid TPS43 tuning block; using production defaults\n");
        tuning = production_tuning();
    ...
    set_configured_tps43_tuning(tuning);
    load_config_v18_or_v19(persisted_config, &config->base, sizeof(persist_config_v24_t));
}

void load_config_v25(const uint8_t* persisted_config) {
    const persist_config_v25_t* config = (const persist_config_v25_t*) persisted_config;
    DualTps43Tuning tuning;
    if (!decode_tps43_tuning(config->tps43_tuning, sizeof(config->tps43_tuning), &tuning)) {
        printf("invalid TPS43 tuning block; using production defaults\n");
        tuning = production_tuning();
    ...
    set_configured_tps43_tuning(tuning);
    load_config_v18_or_v19(persisted_config, &config->base, sizeof(persist_config_v25_t));
}

void load_config_v26(const uint8_t* persisted_config) {
    const persist_config_v26_t* config = (const persist_config_v26_t*) persisted_config;
    DualTps43Tuning tuning;
    if (!decode_tps43_tuning(config->tps43_tuning, sizeof(config->tps43_tuning), &tuning)) {
        printf("invalid TPS43 tuning block; using production defaults\n");
        tuning = production_tuning();
    ...
    set_configured_tps43_tuning(tuning);
    load_config_v18_or_v19(persisted_config, &config->base, sizeof(persist_config_v26_t));
}

void load_config_v27(const uint8_t* persisted_config) {
    const persist_config_v27_t* config = (const persist_config_v27_t*) persisted_config;
    DualTps43Tuning tuning;
    if (!decode_tps43_tuning(config->tps43_tuning, sizeof(config->tps43_tuning), &tuning)) {
        printf("invalid TPS43 tuning block; using production defaults\n");
        tuning = production_tuning();
...
void load_config(const uint8_t* persisted_config) {
    ...
    if (version == 20) {
        ...
        load_config_v23(persisted_config);
    } else if (version == 24) {
        load_config_v24(persisted_config);
    } else if (version == 25) {
...
uint16_t handle_get_report1(uint8_t report_id, uint8_t* buffer, uint16_t reqlen) {
    if (report_id == REPORT_ID_CONFIG && reqlen >= CONFIG_SIZE) {
        ...
        switch (last_config_command) {
            ...
            case ConfigCommand::GET_TPS43_CURSOR_FILTER: {
                if (!get_configured_tps43_cursor_filter(
            ...
            case ConfigCommand::GET_TPS43_SCROLL_GAIN: {
                if (!get_configured_tps43_scroll_gain(
                        (tps43_scroll_gain_tuning_t*) config_buffer->data)) {
                    return 0;
                }
                break;
            }
            case ConfigCommand::GET_TPS43_POWER_MODE_TIMEOUTS: {
                if (!get_configured_tps43_power_mode_timeouts(
                        (tps43_power_mode_timeouts_t*) config_buffer->data)) {
                    return 0;
                }
                break;
            }
            case ConfigCommand::GET_TPS43_SCROLL_DIRECTION: {
                if (!get_configured_tps43_scroll_direction(
                        (tps43_scroll_direction_tuning_t*) config_buffer->data)) {
                    return 0;
                }
                break;
            }
    ...
    if (report_id == REPORT_ID_CONFIG && bufsize >= CONFIG_SIZE) {
        if (checksum_ok(buffer, CONFIG_SIZE) && command_version_ok(buffer)) {
            ...
            switch (config_buffer->command) {
                ...
                case ConfigCommand::SET_TPS43_CURSOR_FILTER: {
                    const tps43_cursor_filter_tuning_t* controls =
                        (const tps43_cursor_filter_tuning_t*) config_buffer->data;
                    if (set_configured_tps43_cursor_filter(*controls)) {
                        tps43_tuning_updated = true;
                    } else {
                        printf("invalid TPS43 cursor filter; retaining previous profile\n");
                    }
                    break;
                }
                case ConfigCommand::SET_TPS43_SCROLL_GAIN: {
                    ...
                    if (set_configured_tps43_scroll_gain(*controls)) {
                        tps43_tuning_updated = true;
                    } else {
                        printf("invalid TPS43 active scroll gain; retaining previous profile\n");
                    }
                    break;
                }
                case ConfigCommand::SET_TPS43_POWER_MODE_TIMEOUTS: {
                    const tps43_power_mode_timeouts_t* timeouts =
                        (const tps43_power_mode_timeouts_t*) config_buffer->data;
                    if (set_configured_tps43_power_mode_timeouts(*timeouts)) {
                        tps43_power_mode_timeouts_updated = true;
                    } else {
                        printf("invalid TPS43 power-mode timeouts; retaining previous settings\n");
                    }
                    break;
                }
                case ConfigCommand::SET_TPS43_SCROLL_DIRECTION: {
                    const tps43_scroll_direction_tuning_t* controls =
                        (const tps43_scroll_direction_tuning_t*) config_buffer->data;
                    if (set_configured_tps43_scroll_direction(*controls)) {
                        tps43_tuning_updated = true;
                    } else {
                        printf("invalid TPS43 scroll direction settings; retaining previous profile\n");
                    }
                    break;
                }
                case ConfigCommand::GET_TPS43_TUNING:
