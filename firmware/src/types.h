...
enum class ConfigCommand : int8_t {
    ...
    GET_TPS43_POWER_MODE_TIMEOUTS = 33,
...
struct __attribute__((packed)) persist_config_v26_t {
    persist_config_v18_t base;
    uint8_t tps43_tuning[102];
};

struct __attribute__((packed)) persist_config_v27_t {
    persist_config_v18_t base;
...
// Two sensor power-mode timeouts. LP1 timeout is represented in 20-second
// register units; the web UI presents seconds and enforces that granularity.
struct __attribute__((packed)) tps43_power_mode_timeouts_t {
    uint8_t idle_timeout_seconds;
    uint8_t lp1_timeout_20s_units;
};
struct __attribute__((packed)) tps43_scroll_direction_tuning_t {
    uint8_t enabled;
    uint8_t classification_distance_counts;
    uint8_t axis_dominance_ratio;
};
