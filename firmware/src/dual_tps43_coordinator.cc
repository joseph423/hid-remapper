...
void DualTps43Coordinator::service(uint64_t now_us) {
    left_driver_.service(now_us);
    right_driver_.service(now_us);

    DualPadSnapshot snapshot;
    snapshot.left = left_tracker_.update(left_driver_.sample());
    snapshot.right = right_tracker_.update(right_driver_.sample());
